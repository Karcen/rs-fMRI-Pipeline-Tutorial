"""
=============================================================================
MULTIMODAL MRI BRAIN CONNECTIVITY PIPELINE  ─  v3
=============================================================================
"""
...
# ─────────────────────────────────────────────────────────────────────────────
PATHS = {
    "t1":  "/Users/karcenzheng/Downloads/Jiacheng_Zheng的3D大脑/nifti/T1",
    ...
    "atlas_nii": "/Users/karcenzheng/Downloads/Jiacheng_Zheng的3D大脑/AAL3v2_for_SPM12/AAL3v1.nii.gz",  # 你的图谱NIfTI文件
    "atlas_labels": "/Users/karcenzheng/Downloads/Jiacheng_Zheng的3D大脑/AAL3v2_for_SPM12/AAL3v1.nii.txt"  # 你的图谱标签文件
}
...
def load_atlas():
    """
    加载自定义NIfTI图谱文件 + 标签文件（替代内置Harvard-Oxford图谱）
    支持的标签文件格式：
    - TXT：每行一个标签（对应图谱的ROI编号，从1开始）
    - CSV：单列（无表头），每行一个标签
    """
    print("\n  Loading CUSTOM atlas (NIfTI + labels) …")

    # 1. 加载图谱NIfTI文件
    atlas_nii_path = PATHS["atlas_nii"]
    if not os.path.exists(atlas_nii_path):
        raise FileNotFoundError(f"自定义图谱文件不存在: {atlas_nii_path}")
    atlas_img = nib.load(atlas_nii_path)
    print(f"    Atlas NIfTI loaded: {atlas_img.shape} (affine: {atlas_img.affine.shape})")

    # 2. 加载图谱标签文件
    atlas_labels_path = PATHS["atlas_labels"]
    if not os.path.exists(atlas_labels_path):
        raise FileNotFoundError(f"图谱标签文件不存在: {atlas_labels_path}")

    # 读取标签（兼容txt/csv）
    if atlas_labels_path.endswith(".txt"):
        with open(atlas_labels_path, "r", encoding="utf-8") as f:
            atlas_labels = [line.strip() for line in f if line.strip()]
    elif atlas_labels_path.endswith(".csv"):
        if not PANDAS_AVAILABLE:
            raise ImportError("需要安装pandas以读取CSV标签文件: pip install pandas")
        df = pd.read_csv(atlas_labels_path, header=None)
        atlas_labels = df[0].astype(str).tolist()
    else:
        raise ValueError("仅支持.txt/.csv格式的标签文件")

    # 3. 验证图谱与标签的匹配性
    atlas_data = atlas_img.get_fdata()
    max_roi_id = int(np.max(atlas_data))
    if len(atlas_labels) != max_roi_id:
        print(f"    [WARN] 标签数量({len(atlas_labels)})与图谱最大ROI编号({max_roi_id})不匹配！")
        # 自动补全/截断标签（保证标签数=ROI数）
        if len(atlas_labels) < max_roi_id:
            for i in range(len(atlas_labels) + 1, max_roi_id + 1):
                atlas_labels.append(f"ROI-{i}")
        else:
            atlas_labels = atlas_labels[:max_roi_id]

    print(f"    Custom atlas loaded: {max_roi_id} ROIs, {len(atlas_labels)} labels")
...
def _align_labels_to_matrix(atlas_labels, n_rois):
    ...
    while len(clean) < n_rois:
        clean.append(f"Custom_ROI-{len(clean) + 1}")
    return clean
...
def build_sc_matrix(streamlines, atlas_img, atlas_labels, dti_affine):
    if not DIPY_AVAILABLE or streamlines is None:
        n_roi = len(atlas_labels)
        return np.zeros((n_roi, n_roi))

    print("  Building structural connectivity matrix (custom atlas) …")

    target_img = nib.load("/Users/karcenzheng/Downloads/Jiacheng_Zheng的3D大脑/nifti/DTI/DTI_WIP_DTI_32dir_ALPS_601.nii.gz")
    ...
    atlas_data = atlas_img_resampled.get_fdata().astype(int)
    atlas_shape = atlas_data.shape
    inv_atlas = np.linalg.inv(atlas_img_resampled.affine)
    ...
    for sl in streamlines:
        if len(sl) < 2:
            continue
        r0 = world_to_label(sl[0])
        r1 = world_to_label(sl[-1])
    ...
    print(f"    SC matrix (custom atlas): {sc.shape} → saved sc_matrix.npy")
    return sc
